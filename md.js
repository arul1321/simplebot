...
const { default: zbotConnect, useSingleFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = require("@adiwajshing/baileys")
...
async function startzbot() {
const zbot = zbotConnect({
...
// anticall auto block
zbot.ws.on('CB:call', async (json) => {
const callerId = json.content[0].attrs['call-creator']
if (json.content[0].tag == 'offer') {
let pa7rick = await zbot.sendContact(callerId, global.owner)
zbot.sendMessage(callerId, { text: `Sistem otomatis block!\nJangan menelpon bot!\nSilahkan Hubungi Owner Untuk Dibuka !`}, { quoted : pa7rick })
await sleep(8000)
await zbot.updateBlockStatus(callerId, "block")
}
})
...
require("./command/zbot")(zbot, m, chatUpdate, store)
} catch (err) {
console.log(err)
}
})
...
else if (reason === DisconnectReason.connectionClosed) { console.log("Connection closed, reconnecting...."); startzbot(); }
else if (reason === DisconnectReason.connectionLost) { console.log("Connection Lost from Server, reconnecting..."); startzbot(); }
...
else if (reason === DisconnectReason.restartRequired) { console.log("Restart Required, Restarting..."); startzbot(); }
else if (reason === DisconnectReason.timedOut) { console.log("Connection TimedOut, Reconnecting..."); startzbot(); }
else zbot.end(`Unknown DisconnectReason: ${reason}|${connection}`)
}
console.log('Connected...', update)
})

zbot.ev.on('creds.update', saveState)
...
* @returns
*/
zbot.send5ButImg = async (jid , text = '' , footer = '', img, but = [], options = {}) =>{
let message = await prepareWAMessageMedia({ image: img }, { upload: zbot.waUploadToServer })
...
zbot.relayMessage(jid, template.message, { messageId: template.key.id })
}
...
* @returns 
*/
zbot.sendText = (jid, text, quoted = '', options) => zbot.sendMessage(jid, { text: text, ...options }, { quoted })
...
* @returns 
*/
zbot.sendTextWithMentions = async (jid, text, quoted, options = {}) => zbot.sendMessage(jid, { text: text, contextInfo: { mentionedJid: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net') }, ...options }, { quoted })
